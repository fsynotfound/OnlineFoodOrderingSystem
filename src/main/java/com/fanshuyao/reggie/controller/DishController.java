...
public class DishController {
     ...
     */
    @PostMapping
    public R<String> save(@RequestBody DishDto dishDto){ //提交过来是json数据，因此要加上requestbody
    ...
    public R<Page> page(int page, int pageSize, String name){
        ...
        //对象拷贝，但此时需要忽略pageInfo里的records
        BeanUtils.copyProperties(pageInfo,dishDtoPage,"records");

        List<Dish> records = pageInfo.getRecords();

        List<DishDto> list = records.stream().map((item) -> { //item代表的是dish,即遍历出来的每一个菜品对象
            ...
            return dishDto;
        }).collect(Collectors.toList()); //把所有的dto对象收集起来转成集合
    ...
    /**
     * 根据id查询菜品信息和对应的口味信息，此页面和新增菜品用的是同一个页面，因为前端返回的内容有data,所以要返回实体类
     * @param id
     * @return
     */
    @GetMapping("/{id}")
     ...
     * @return
     */
    /*@GetMapping("/list")  按下"command+shift+/"将以下内容注释掉
    ...
    public R<List<DishDto>> list(Dish dish){
        ...
        List<DishDto> dishDtoList = list.stream().map((item) -> { //item表示每一个dish对象
            DishDto dishDto = new DishDto();
